...
rest = on_fullmatch("重置状态", priority=25, permission=GROUP, block=True)
...
@rest.handle(parameterless=[Cooldown(at_sender=False)])
async def rest_(bot: Bot, event: GroupMessageEvent):
    """修炼"""
...
@exp_up.handle(parameterless=[Cooldown(at_sender=False)])
async def exp_up_(bot: Bot, event: GroupMessageEvent):
    ...
    if not is_type:
        ...
        exp_time = random.randint(1,4) * 3  # 闭关时长计算(分钟) = second // 60
        sleep_time = exp_time * 10
        if sleep_time <= 30:
            msg = f"屏息凝神，感受到星星点点的道韵流动，有少许感悟，进入{int(sleep_time)}秒修炼"
        elif sleep_time <= 60:
            msg = f"屏息凝神，感受到连绵不绝的道韵在周身流动，颇有感悟，进入{int(sleep_time)}秒修炼"
        else:
            msg = f"屏息凝神，感受到了浩如烟海的道韵涌入脑海，突生顿悟，进入{int(sleep_time)}秒修炼"
        await bot.send_group_msg(group_id=int(send_group_id), message=msg)
        ...
        if exp >= user_get_exp_max:
            ...
            msg = f"修炼结束，本次修炼触及瓶颈，共增加修为：{number_to(user_get_exp_max)}{result_msg[0]}{result_msg[1]}"
            await bot.send_group_msg(group_id=int(send_group_id), message=msg)
            await exp_up.finish()
        else:
            sql_message.in_closing(user_id, user_type)
            ...
            result_msg, result_hp_mp = OtherSet().send_hp_mp(user_id, int(exp * hp_speed * (1 + mainbuffclors)),
                                                             int(exp * mp_speed))
            sql_message.update_user_attribute(user_id, result_hp_mp[0], result_hp_mp[1], int(result_hp_mp[2] / 10))
            msg = f"修炼结束，本次修炼增加修为：{number_to(exp)}{result_msg[0]}{result_msg[1]}"
            await bot.send_group_msg(group_id=int(send_group_id), message=msg)
            await exp_up.finish()
