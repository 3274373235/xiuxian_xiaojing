...
async def impart_pk_now_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    player_1_stones = 100000000
    player_2_stones = 100000000
    combined_msg = ""
    duel_count = 0
    ...
    if not num:
        while user_data["pk_num"] > 0:
            ...
            if win == 1:
                msg += f"战报：道友{user_info['user_name']}获胜,获得思恋结晶100000000颗\n"
                impart_pk.update_user_data(user_info['user_id'], True)
                xiuxian_impart.update_stone_num(100000000, user_id, 1)
                player_1_stones += 100000000
            elif win == 2:
                msg += f"战报：道友{user_info['user_name']}败了,消耗一次次数,获得思恋结晶100000000颗\n"
    ...
    while user_data["pk_num"] > 0:
        ...
        if win == 1:  # 1号玩家胜利 发起者
            ...
            xiuxian_impart.update_stone_num(100000000, player_2, 1)
            xiuxian_impart.update_stone_num(100000000, player_1, 1)
            player_2_stones += 100000000
            player_1_stones += 100000000
            msg_list.append(
                {"type": "node", "data": {"name": f"虚神界战报", "uin": bot.self_id,
