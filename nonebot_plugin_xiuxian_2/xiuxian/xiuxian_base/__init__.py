...
rank = on_command("排行榜", aliases={"修仙排行榜", "灵石排行榜", "轮回排行榜", "战力排行榜", "境界排行榜", "宗门排行榜"},
...
level_up_lxzj = on_fullmatch("连续突破",  priority=7, permission=GROUP, block=True)
...
restate = on_command("重置状态11356", permission=SUPERUSER, priority=12, block=True)
set_xiuxian = on_command("启用修仙功能", aliases={'禁用修仙功能'}, permission=GROUP and (GROUP | SUPERUSER | GROUP_ADMIN | GROUP_OWNER), priority=5, block=True)
...
async def rank_(bot: Bot, event: GroupMessageEvent):
    ...
    if message in ["排行榜", "修仙排行榜", "境界排行榜", "修为排行榜"]:
        ...
        c_rank = sql_message.lunhui_top()
        msg = f"✨位面轮回排行榜TOP50✨\n"
        num = 0
        for i in c_rank:
            num += 1
            msg += f"第{num}位  {i[0]}  轮回次数：{number_to(i[1])}\n"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
...
# 拆分境界名称为阶段和层次
def get_stage_and_tier(level_name):
    for stage in stage_order:
        if level_name.startswith(stage):
            tier = level_name[len(stage):]
            # 去掉括号
...
# 获取境界的排序键
def get_sort_key(level_name):
    stage, tier = get_stage_and_tier(level_name)
    return stage_order.index(stage), tier_order.get(tier, 0)

# 获取所有境界名称并排序
levels = jsondata.level_rate_data().keys()
sorted_levels = sorted(levels, key=get_sort_key)

@level_up_lxzj.handle(parameterless=[Cooldown(stamina_cost=6, at_sender=False)])
async def level_up_lxzj_(bot: Bot, event: GroupMessageEvent):
    """连续直接突破五次"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    isUser, user_info, msg = check_user(event)
    if not isUser:
    ...
    current_level = user_msg['level']
    current_index = sorted_levels.index(current_level)
    success_count = 0  # 记录成功突破的次数
    failed_count = 0  # 记录失败突破的次数
    result_msg = []  # 存储每次突破的结果消息
    
    user_leveluprate = int(user_msg['level_up_rate'])  # 获取初始成功率
    
    for _ in range(5):  # 连续突破五次
        if current_index >= len(sorted_levels) - 1:
            result_msg.append("道友已经是最高境界，无法再突破了！")
            break  # 如果已经是最高境界，直接退出循环
        
        next_level = sorted_levels[current_index + 1]
        # 模拟突破成功率
        level_rate = jsondata.level_rate_data()[current_level]
        main_rate_buff = UserBuffDate(user_id).get_user_main_buff_data()
        number = main_rate_buff['number'] if main_rate_buff else 0
        success_probability = level_rate + user_leveluprate + number
        
        if random.randint(1, 100) <= success_probability:  # 突破成功
            ...
            sql_message.update_user_hp(user_id)
            result_msg.append(f"恭喜道友突破至{next_level}成功！")
            success_count += 1
            current_index += 1  # 更新当前境界索引
            current_level = next_level  # 更新当前境界
            user_leveluprate = 0  # 成功后重置成功率
            user_msg = sql_message.get_user_info_with_id(user_id)  # 更新用户信息
        else:  # 突破失败
            user_exp = int(user_msg['exp'])
            扣除值 = max(1, int(user_exp * 0.1))
            if 扣除值 >= user_exp and user_exp <= 9:
                result_msg.append("当前修为不足以突破，请修炼后再尝试")
                break  # 退出循环
            else:
                sql_message.update_j_exp(user_id, 扣除值)
                nowhp = user_msg['hp'] - (扣除值 / 2) if (user_msg['hp'] - (扣除值 / 2)) > 0 else 1
                nowmp = user_msg['mp'] - 扣除值 if (user_msg['mp'] - 扣除值) > 0 else 1
                sql_message.update_user_hp_mp(user_id, nowhp, nowmp)
                update_rate = 1
                user_leveluprate += update_rate
                sql_message.update_levelrate(user_id, user_leveluprate)
                result_msg.append(f"道友突破至{next_level}失败，修为减少{扣除值}，下次突破成功率增加{update_rate}%！")
                failed_count += 1
                user_msg = sql_message.get_user_info_with_id(user_id)  # 更新用户信息
    # 汇总突破结果
    summary_msg = f"连续突破五次结果：\n"
    summary_msg += f"成功次数：{success_count}，失败次数：{failed_count}\n"
    summary_msg += "\n".join(result_msg)
    ...
    if XiuConfig().img:
        ...
        await bot.send_group_msg(group_id=int(send_group_id), message=summary_msg)
    await level_up_zj.finish()
# 在突破代码中使用排序后的境界列表
...
async def level_up_zj_(bot: Bot, event: GroupMessageEvent):
    ...
    if current_index < len(sorted_levels) - 1:
        ...
        # 获取用户的成功率加成
        user_leveluprate = int(user_msg['level_up_rate'])
        ...
        if random.randint(1, 100) <= success_probability:
            ...
            exp_buff = main_exp_buff['exp_buff'] if main_exp_buff else 0
            now_exp = int(int(user_msg['exp']) * ((percentage / 100) * (1 - exp_buff)))
            ...
            msg = f"道友突破至{next_level}失败，修为减少{now_exp}，下次突破成功率增加{update_rate}%！"
    else:
        # 已经是最高境界
        msg = "道友已经是最高境界，无法再突破了！"
...
async def level_up_drjd_(bot: Bot, event: GroupMessageEvent):
    ...
    pause_flag = any(int(back['goods_id']) == 1998 for back in user_backs) if user_backs else False
    ...
    if current_index < len(sorted_levels) - 1:
        ...
        sql_message.update_levelrate(user_id, 0)
        sql_message.update_user_hp(user_id)
        now_exp = int(int(user_msg['exp']) * 0.1)
        sql_message.update_exp(user_id, now_exp)
        msg = f"恭喜道友突破至{next_level}成功，因为使用了渡厄金丹，修为也增加了一成！！"
    else:
        # 已经是最高境界
        msg = "道友已经是最高境界，无法再突破了！"
...
async def level_up_dr_(bot: Bot, event: GroupMessageEvent):
    ...
    elixir_name = "渡厄丹"
    user_backs = sql_message.get_back_msg(user_id)
    pause_flag = any(int(back['goods_id']) == 1999 for back in user_backs) if user_backs else False
