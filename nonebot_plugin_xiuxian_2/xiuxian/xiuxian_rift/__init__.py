...
set_group_rift = on_command("群秘境", priority=4, permission=GROUP and (GROUP | SUPERUSER | GROUP_ADMIN | GROUP_OWNER), block=True)
explore_rift = on_command("探索秘境", aliases={"探索"} , priority=5, permission=GROUP, block=True)
rift_help = on_fullmatch("秘境帮助", priority=6, permission=GROUP, block=True)
create_rift = on_fullmatch("生成秘境", priority=5, permission=GROUP and (GROUP | SUPERUSER | GROUP_ADMIN | GROUP_OWNER), block=True)
    ...
    if not is_type:
        ...
        try:
            ...
            if XiuConfig().img:
                ...
                await bot.send_group_msg(group_id=int(send_group_id), message=msg)
            await explore_rift.finish()
        if 00000000000000 in group_rift[group_id].l_user_id:
