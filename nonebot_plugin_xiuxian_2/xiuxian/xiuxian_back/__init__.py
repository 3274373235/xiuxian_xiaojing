...
from .back_util import (
    get_user_main_back_msg, get_yaocai, get_danyao, check_equipment_can_use,
...
xg_item = on_command('查看效果', aliases={'效果'}, priority=10, permission=GROUP, block=True)
main_back = on_command('我的背包', aliases={'背包'}, priority=10, permission=GROUP, block=True)
yaocai_back = on_command('药材背包', aliases={'药材'}, priority=10, permission=GROUP, block=True)
danyao_back = on_command('丹药背包', aliases={'丹药'}, priority=10, permission=GROUP, block=True)
...
import json
@xg_item.handle()
async def xg_item_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    # 去掉“查看效果”并去掉前后空格
    args_str = args.extract_plain_text().replace("查看效果", "").strip()
    
    if not args_str:
        await bot.send(event, "请输入物品名称。")
        return
    
    # 加载item.json文件
    try:
        with open('C:/Users/Administrator/Desktop/bot/xiaojing/data/xiuxian/items.json', 'r', encoding='utf-8') as f:
            item_data = json.load(f)
    except Exception as e:
        await bot.send(event, f"读取物品数据失败: {e}")
        return
    
    # 遍历字典的值，找到name等于args_str的项
    for good_id, item_info in item_data.items():
        if item_info.get('name') == args_str:
            # 调用get_item_msg函数
            msg = get_item_msg(good_id)
            await bot.send(event, msg)
            return
    
    # 如果没有找到匹配的物品
    await bot.send(event, f"未找到物品：{args_str}")
...
async def main_back_(bot: Bot, event: GroupMessageEvent):
    ...
    user_id = user_info['user_id']    
    # 调用三个函数获取不同部分的消息列表
    msg_list1 = get_user_main_back_msg(user_id)
    # 合并所有消息列表
    msg_list = msg_list1   
    if XiuConfig().img:
        pic = await get_msg_pic(f"@{event.sender.nickname}\n" + "\n".join(msg_list))
        ...
        await bot.send_group_msg(group_id=int(send_group_id), message="\n".join(msg_list))
    await main_back.finish()
...
async def danyao_back_(bot: Bot, event: GroupMessageEvent):
    """丹药背包"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    isUser, user_info, msg = check_user(event)
    if not isUser:
