...
async def Boss_fight(player1: dict, boss: dict, type_in=2, bot_id=0):
    ...
    if convert_rank('混沌境初期')[0] < convert_rank((boss["jj"] + '中期'))[0] < convert_rank('江湖好手')[0]: #遁一以下
            boss["减伤"] = 1 # boss减伤率
            boss_zs = 0
            boss_hx = 0
            boss_bs = 0
    ...
    if convert_rank('大乘初期')[0] < convert_rank((boss["jj"] + '中期'))[0] < convert_rank('至尊境圆满')[0]: #真仙境
    ...
    if convert_rank('金仙境初期')[0] < convert_rank((boss["jj"] + '中期'))[0] < convert_rank('真仙境圆满')[0]: #仙王境 
    ...
    if convert_rank('轮回境初期')[0] < convert_rank((boss["jj"]+ '中期'))[0] < convert_rank('仙王境圆满')[0]: #准帝境
    ...
    if convert_rank('渡劫初期')[0] < convert_rank((boss["jj"]+ '中期'))[0] < convert_rank('准帝境圆满')[0]: #仙帝境
            boss["减伤"] = 0.1  # boss减伤率
            boss_st1 = random.randint(0,100) #boss神通1
            if 0 <= boss_st1 <= 25:
                boss_zs = 0.9   #boss攻击
