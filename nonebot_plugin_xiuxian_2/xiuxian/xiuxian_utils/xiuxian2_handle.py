...
class XiuxianDateManage:
    ...
    def update_user_unlimited_root(self, user_id, root_name, root_speed, text):
        sql = "UPDATE user_xiuxian SET unlimited_root_name=?, unlimited_root_speed=? WHERE user_id=?"
        sql1 = "UPDATE user_xiuxian SET root=? WHERE user_id=?"
        self.cursor.execute(sql, (root_name, root_speed, user_id))
        self.cursor.execute(sql1, (text, user_id))
        self.conn.commit()

    def update_user_unlimited_root_speed(self, user_id, root_speed):
        sql = "UPDATE user_xiuxian SET unlimited_root_speed=? WHERE user_id=?"
        self.cursor.execute(sql, (root_speed, user_id))
        self.conn.commit()

    def increment_unlimited_loop_count(self, user_id):
        sql = "UPDATE user_xiuxian SET unlimited_loop_count=unlimited_loop_count+1 WHERE user_id=?"
        self.cursor.execute(sql, (user_id,))
        self.conn.commit()

    def update_user_root_type(self, user_id, new_root_type):
       try:
           sql = "UPDATE user_xiuxian SET root_type=? WHERE user_id=?"
           self.cursor.execute(sql, (new_root_type, user_id))
           self.conn.commit()
...
WHERE last_check_info_time = '0' OR last_check_info_time IS NULL
    ...
    def update_root(self, user_id, key):
        """更新灵根  1为混沌,2为融合,3为超,4为龙,5为天,6为千世,7为万世,8为无尽轮回"""
        cur = self.conn.cursor()
        root_name = ""
        if int(key) == 1:
            ...
            sql = f"UPDATE user_xiuxian SET root=?, root_type=? WHERE user_id=?"
            cur.execute(sql, ("永恒轮回，无尽升华", "永恒道果", user_id))
            root_name = "永恒道果"
        self.conn.commit()
        return root_name
    ...
    def lunhui_top(self):
        """轮回排行榜"""
        sql = f"SELECT user_name,unlimited_loop_count FROM user_xiuxian WHERE user_name is NOT NULL ORDER BY unlimited_loop_count DESC LIMIT 50"
        cur = self.conn.cursor()
        cur.execute(sql, )
        result = cur.fetchall()
        return result
