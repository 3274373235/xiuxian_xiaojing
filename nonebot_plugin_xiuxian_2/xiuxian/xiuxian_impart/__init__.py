...
impart_draw1 = on_command("千次祈愿", priority=16, permission=GROUP, block=True)
impart_draw = on_command("传承祈愿", priority=16, permission=GROUP, block=True)
...
@impart_draw1.handle(parameterless=[Cooldown(at_sender=False)])
async def impart_draw1_(bot: Bot, event: GroupMessageEvent):
    """传承抽卡1000次"""
    bot, send_group_id = await assign_bot(bot=bot, event=event)
    ...
    if impart_data_draw is None:
        ...
        if XiuConfig().img:
            ...
            await bot.send_group_msg(group_id=int(send_group_id), message=msg)
        await impart_draw1.finish()
    if impart_data_draw['stone_num'] < 10000:
        msg = f"思恋结晶数量不足10000个,无法抽卡1000次!"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
        ...
        xiuxian_impart.add_impart_exp_day(total_exp_day, user_id)
        xiuxian_impart.update_stone_num(10000, user_id, 2)
        xiuxian_impart.update_impart_wish(0, user_id)
        await re_impart_data(user_id)

        msg = f"道友成功抽卡1000次，累计获得闭关时间：{total_exp_day}分钟！\n"
        if new_cards:
            msg += f"获得新的传承卡片：{', '.join(new_cards)}"
        if XiuConfig().img:
            pic = await get_msg_pic(f"@{event.sender.nickname}\n" + msg)
            await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment.image(pic))
        else:
