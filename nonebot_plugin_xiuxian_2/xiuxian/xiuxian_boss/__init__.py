...
set_group_boss = on_command("世界boss", aliases={"世界Boss", "世界BOSS"}, priority=13,
                            permission=GROUP and (GROUP | SUPERUSER | GROUP_ADMIN | GROUP_OWNER), block=True)
...
async def battle_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    boss_rank = boss_rank if boss_rank is not None else 0
    user_rank = user_rank if user_rank is not None else 0
